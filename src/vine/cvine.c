               ...
               dml_copula_selection_t selection)
{
    ...
    for (k = 0; k < n - 1; k++) { // Tree index.
        ...
        tau = g_malloc_n(n - k, sizeof(dml_measure_tau_t **));
        for (size_t i = 0; i < n - k; i++) {
            tau[i] = g_malloc0_n(n - k, sizeof(dml_measure_tau_t *));
        }
        ...
        for (a = 0; a < igraph_vcount(trees[k]); a++) {
            ...
            for (b = 0; b < igraph_vcount(trees[k]); b++) {
                if (a != b) {
                    ...
                    if (tau[(size_t) a][(size_t) b] == NULL) {
                        tau[(size_t) a][(size_t) b] = dml_measure_tau_alloc(xa, xb);
                        tau[(size_t) b][(size_t) a] = tau[(size_t) a][(size_t) b];
                    }
                    tree_weight += cvine_calculate_weight(
                            weight, tau[(size_t) a][(size_t) b], xa, xb);
                }
            }
...
/* Based on Algorithm 1 of Aas, K., Czado, C., Frigessi, A. and Bakken, H.
